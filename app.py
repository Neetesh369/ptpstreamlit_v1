...
if "long_entry_z" not in st.session_state:
    st.session_state.long_entry_z = -2.5
if "long_exit_z" not in st.session_state:
    st.session_state.long_exit_z = -1.5
if "short_entry_z" not in st.session_state:
    st.session_state.short_entry_z = 2.5
if "short_exit_z" not in st.session_state:
    st.session_state.short_exit_z = 1.5
...
# --- Long Entry Z-Score ---
with col1:
    st.write("**Long Entry Z**")
    if st.button("➕ Long Entry"):
        if st.session_state.long_entry_z < 0:
            st.session_state.long_entry_z += 0.1
            st.session_state.long_entry_z = round(st.session_state.long_entry_z, 2)
    if st.button("➖ Long Entry"):
        st.session_state.long_entry_z -= 0.1
        st.session_state.long_entry_z = round(st.session_state.long_entry_z, 2)
    st.write(f"Value: {st.session_state.long_entry_z}")

# --- Long Exit Z-Score ---
with col2:
    st.write("**Long Exit Z**")
    if st.button("➕ Long Exit"):
        # Only allow increase if still > long entry
        proposed = round(st.session_state.long_exit_z + 0.1, 2)
        if proposed > st.session_state.long_entry_z:
            st.session_state.long_exit_z = proposed
    if st.button("➖ Long Exit"):
        # Only allow decrease if still > long entry
        proposed = round(st.session_state.long_exit_z - 0.1, 2)
        if proposed > st.session_state.long_entry_z:
            st.session_state.long_exit_z = proposed
    st.write(f"Value: {st.session_state.long_exit_z}")

# --- Short Entry Z-Score ---
with col3:
    st.write("**Short Entry Z**")
    if st.button("➕ Short Entry"):
        st.session_state.short_entry_z += 0.1
        st.session_state.short_entry_z = round(st.session_state.short_entry_z, 2)
    if st.button("➖ Short Entry"):
        if st.session_state.short_entry_z > 0:
            st.session_state.short_entry_z -= 0.1
            st.session_state.short_entry_z = round(st.session_state.short_entry_z, 2)
    st.write(f"Value: {st.session_state.short_entry_z}")

# --- Short Exit Z-Score ---
with col4:
    st.write("**Short Exit Z**")
    if st.button("➕ Short Exit"):
        # Only allow increase if still < short entry
        proposed = round(st.session_state.short_exit_z + 0.1, 2)
        if proposed < st.session_state.short_entry_z:
            st.session_state.short_exit_z = proposed
    if st.button("➖ Short Exit"):
        st.session_state.short_exit_z -= 0.1
        st.session_state.short_exit_z = round(st.session_state.short_exit_z, 2)
    st.write(f"Value: {st.session_state.short_exit_z}")
